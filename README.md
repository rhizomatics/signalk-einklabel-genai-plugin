...
Install both plugins from the SignalK AppStore (or `npm install @rhizomatics/signalk-einklabel-plugin @rhizomatics/signalk-einklabel-genai-plugin`), then install whichever LLM provider package(s) you'll actually use - these are `optionalDependencies`, not installed automatically, since a user only ever needs one:
...
- `llmProvider` - `openai`, `anthropic`, `google`, `xai`, `deepseek`, `moonshotai`, `ollama`, or `local` (any other OpenAI-compatible server)
...
```bash
esl-cli -r node_modules/@rhizomatics/signalk-einklabel-genai-plugin/dist/cliExtension.js prompt forecast -e examples
esl-cli -r node_modules/@rhizomatics/signalk-einklabel-genai-plugin/dist/cliExtension.js generate forecast -e examples -o forecast.png --llm-provider ollama --llm-model llama3.2
```
...
This plugin's own `package.json` currently points its `@rhizomatics/signalk-einklabel-plugin` dependency at a local `file:` path for development against an unpublished core plugin - update it to a real semver range once both are published.
